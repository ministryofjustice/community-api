...
public interface OGRSAssessmentRepository extends JpaRepository<OGRSAssessment, Long> {
    ...
    @Query("select assessment from OGRSAssessment assessment " +
        "inner join assessment.event event " +
        "where event.offenderId = :offenderId " +
        "and event.softDeleted = 0 " +
        "and assessment.softDeleted = 0 " +
