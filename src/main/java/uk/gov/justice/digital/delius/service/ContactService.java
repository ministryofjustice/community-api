...
public class ContactService {
    ...
    public Optional<ContactReport> contactReportFor(Long offenderId) {
        ...
        List<Long> linkedContactIDs = contacts.stream().filter(contact -> contact.getLinkedContactId() != null).map(c -> c.getLinkedContactId()).collect(Collectors.toList());
        ...
        List<uk.gov.justice.digital.delius.data.api.Event> events = contactsGroupedByEvent.entrySet().stream().map(entry ->
                uk.gov.justice.digital.delius.data.api.Event.builder()
                        .eventId(entry.getKey().getEventId())
        ...
        List<uk.gov.justice.digital.delius.data.api.Contact> linkedContacts = contactRepository.findAll(linkedContactIDs).stream().map(contactTransformer::contactOf).collect(Collectors.toList());
