...
public class ContactControllerSecure {
    ...
    @ApiResponses(
        value = {
            @ApiResponse(code = 200, message = "Contact with matching id and crn", response = ContactSummary.class),
    ...
    @ApiOperation(value = "Gets offender contact summary by CRN & contact id")
    public ContactSummary getOffenderContactSummaryByCrn(final @PathVariable("crn") String crn, final @PathVariable("contactId") Long contactId) {
        ...
        return contactService.getContactSummary(offenderId, contactId).orElseThrow(() ->
